...
void PhoneBook::DeleteByName()
{
    ...
    if (index >= 0)
    {
        ...
        /*{
            memcpy(contacts_[i].name, contacts_[i + 1].name, sizeof(contacts_[i].name));
            memcpy(contacts_[i].phone, contacts_[i + 1].phone, sizeof(contacts_[i].phone));
        }*/   //내가 푼 풀이
        {
            memcpy(&contacts_[i], &contacts_[i+1], sizeof(Contact));
        }   // 위와 같이 요약이 가능하다.
        
        /*contacts_[num_contacts_].name[0] = '\0'; // 내가 한 풀이 : 필요없는 부분이었다.
